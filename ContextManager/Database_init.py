...
class DB_Creation:
    ...
    create_file_table = "CREATE TABLE IF NOT EXISTS Files(Id INTEGER PRIMARY KEY AUTOINCREMENT, Hash TEXT, File_path TEXT, Size TEXT, Type TEXT);" 
    # Might not use table below
    # create_uploaded_files_table = "CREATE TABLE IF NOT EXISTS Uploaded_Files(Id INTEGER PRIMARY KEY AUTOINCREMENT, File_id INTEGER, Name TEXT, User TEXT, Date_of_upload DATE, Filtered BOOLEAN, FOREIGN KEY(File_id) REFERENCES Files(Id))"
    ...
    #Below is new files in chat table to cope with file hashing
    create_files_in_chat_table = "CREATE TABLE IF NOT EXISTS Files_in_Chat(Id INTEGER PRIMARY KEY AUTOINCREMENT, Chat_id TEXT, File_id INTEGER, User TEXT, Upload_date DATE, File_name TEXT, FOREIGN KEY(File_id) REFERENCES Files(Id));"
    # Used to keep track of which files are in which chat. 
    create_current_file_table = "CREATE TABLE IF NOT EXISTS Current_file(Id INTEGER PRIMARY KEY AUTOINCREMENT, Chat_id TEXT, File_id INT, Type TEXT, FOREIGN KEY(File_id) REFERENCES Files_in_Chat(Id) ON DELETE CASCADE);"
...
c_string = config('CONNECTIONSTRING')
db = DB_Creation(c_string)
db.create_tables()